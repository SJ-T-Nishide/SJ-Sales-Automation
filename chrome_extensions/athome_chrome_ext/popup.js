...
function isAtHomePage(tab) {
  return tab && tab.url && tab.url.includes('athome.co.jp');
}
...
async function extractCurrentPage() {
  ...
  if (!isAtHomePage(tab)) {
    setStatus('atHomeの検索結果ページを開いてください', 'warn');
    return;
  }
...
async function copyTSV() {
  ...
  setStatus(records.length + '件をコピーしました - スプレッドシート受信タブに貼り付けてください', 'ok');
}
...
async function debugOutput() {
  const tab = await getActiveTab();
  if (!isAtHomePage(tab)) {
    setStatus('atHomeのページを開いてください', 'warn');
    return;
  }
